...
async def fetch_monthly_data_1(month, year):
...
async def fetch_all_data_1():
    ...
    for year in range(start_year, end_year + 1):
        for month in months:
            ...
            month_data = await fetch_monthly_data_1(month, year)
...
async def fetch_monthly_data_2(month, year):
    url = f'https://www.indgold.com/{city}-gold-rate-{month}-{year}.htm'
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page()
...
async def fetch_all_data_2():
    ...
    for year in range(2023,2025):
        for month in months:
            ...
            month_data = await fetch_monthly_data_2(month, year)
